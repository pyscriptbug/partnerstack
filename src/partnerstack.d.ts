declare namespace Partnerstack {
   ...
   * For instance, if you make a list request and receive 100 items,
   * then the last item with "key":"part_f4Jmxb7LmvhERZ", can be used in your subsequent call as the parameter value
   * to fetch the next page of results: ?starting_after=part_f4Jmxb7LmvhERZ.
   */
  export type QueryParams<T = unknown> = {
    ...
    groupKey?: string;
    /**
     * Will filter to a specific partnership's partner_key, which can be found using the partnership's get
     * endpoint or from the partnership view in your PartnerStack dashboard.
     */
    partnerKey?: string;
    ...
    limit?: number;
    /**
     * A cursor for use in pagination, starting_after takes in an item key and the subsequent call will return the following items in the list.
     * This is mutually exclusive with ending_before.
     */
    startingAfter?: string;
    /**
     * A cursor for use in pagination, ending_before takes in an item key and the subsequent call will return the prior items in the list.
